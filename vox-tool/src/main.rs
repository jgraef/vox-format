...
enum Args {
    ...
    Strip {
        /// Selects chunks to strip. If omitted, everything except chunks specified with `--keep` will be stripped.
        #[structopt(short = "s", long = "strip")]
        strip: Vec<String>,

        /// Keep the specified chunks. If this and `--strip` is omitted, every chunk except `PACK`, `XYZI`, `RGBA` and `MATT` is stripped.
        #[structopt(short = "k", long = "keep")]
        keep: Vec<String>,

        /// Output file. Defaults to `INPUT.stripped.vox` where `INPUT` is the input file path without `.vox` file extension.
...
impl Args {
    fn run(self) -> Result<(), Error> {
        match self {
            Self::Strip { strip, keep, input, output } => {
